...
abstract contract BondBaseFPA is IBondFPA, Auth {
    ...
    function _createMarket(MarketParams memory params_) internal returns (uint256) {
        ...
        // Check that deposit interval is in bounds
        ...
        uint256 _maxPayout = (
            params_.capacityInQuote
                ...
                : params_.capacity
        ).mulDiv(uint256(params_.depositInterval), uint256(length));
        ...
        terms[marketId] = BondTerms({vesting: params_.vesting, conclusion: params_.conclusion});
    ...
    function purchaseBond(
        ...
        if (currentTime >= terms[id_].conclusion)
            revert Auctioneer_MarketConcluded(terms[id_].conclusion);
