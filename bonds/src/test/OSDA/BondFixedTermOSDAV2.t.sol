...
import {BondFixedTermOSDA} from "../../BondFixedTermOSDA.sol";
...
// V2 test suite uses a callback for payout tokens
contract BondFixedTermOSDAV2Test is Test {
    ...
    BondFixedTermOSDA internal auctioneer;
    ...
    function setUp() public {
        ...
        auctioneer = new BondFixedTermOSDA(teller, aggregator, guardian, auth);
    ...
    function testCorrectness_BondTokensIssued(
        ...
        priceFeed.setTimestamp(block.timestamp + 86400);
        vm.prank(alice);
    ...
    function testCorrectness_isInstantSwap() public {
        ...
        // First market has a vesting period of 7 days, so it should not be instant swap
        ...
        uint256 id3 = auctioneer.createMarket(
            abi.encode(
                IBondOSDA.MarketParams(
                    ...
                    uint48(14 days), // uint48 vesting (timestamp or duration)
                    uint48(block.timestamp + 7 days) // uint48 conclusion (timestamp)
                )
            )
        );
